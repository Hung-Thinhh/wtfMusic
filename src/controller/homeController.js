...
const handleHome = async (req, res) => {
  try {
    ...
    const playlist = await ZingMp3.getDetailPlaylist("6BD60BOE");
    ...
    //     const song = await ZingMp3.getInfoSong(listSong[i]);
    //     if (song && song.data) {
    ...
    //       let link = await ZingMp3.getSong(song.data.encodeId);

    //       const lyric = await ZingMp3.getLyric(song.data.encodeId);
    //       let form = {
    //         infor: {
    //           id: song.data.encodeId,
    //           songname: song.data.title,
    ...
    //           alias: song.data.alias,
    //           artists: artistIds,
    //           genresid: song.data.genreIds,
    //           songLink: link && link.data && link.data["128"],
    //           duration: song.data.duration,
    //           lyric: lyric.data && lyric.data["sentences"],
    //           like: getRandomNumber(100, 5000),
    //           listen: getRandomNumber(100, 5000),
    //         },
    //       };
    //       if (link && link.data && link.data["128"]) {
    //         songid.push(song.encodeId);
    //         // console.log(song.encodeId);
    //         await Song.findOneAndUpdate({ id: form.infor.id }, form.infor, {
    //           upsert: true,
    //           new: true,
    //         });
    //       }
