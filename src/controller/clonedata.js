...
const fetchclone = async (_, res) => {
  try {
    try {
      const response = await Nuxtify.song.getDetail('Z6DOU8ZC');
      const Lyric = await Nuxtify.song.getLyrics('Z6DOU8ZC');
      const LinkSong = await Nuxtify.song.getUrl('Z6DOU8ZC');
      if (response && response.data) {
        ...
        console.log(song.like)
          ...
          const jj = {
            ...
            try {
                const updatedSong = await Song.findOneAndUpdate({ id: jj.id }, jj, { new: true, upsert: true });
...
const fetchArtistsClone = async (req, res) => {
  try {
    ...
    const updatedartists = await Ar.findOneAndUpdate({ id: id }, artistsData, {
      upsert: true,
    });
