import {
  ...
  setSilde,
  insertSilde,
} from "../../services/adminService/getSlide.js";
      ...
      try {
        ...
        if (!file) {
          ...
          const slide = {
            ...
            playlistId:
              data.playlistId.length > 16
                ? data.playlistId.split(",")
                : data.playlistId, // Convert to array
          };
          try {
        ...
        return res.status(400).json({ error: error.message+'1111' });
      }
    });
  } catch (error) {
    return res.status(400).json({ error: error.message+'22222' });
  }
};
